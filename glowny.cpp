...
long dlugosc_dnia = 500;         // czas trwania dnia w [s]
...
DWORD WINAPI FunkcjaWatkuOdbioru(void *ptr)
{
	...
	while (1)
	{
		...
		case ZAMIANA: {
			...
			if (ramka.iID != moj_pojazd->iID) {
				//fprintf(f, "Zamiana %d\n", moj_pojazd->iID);
				//fprintf(f, "Czy gotów %d\n", moj_pojazd->czy_gotowy_na_zmiane);
				fprintf(f, "ramka do zmiant %d\n", ramka.doZmiany);
				fprintf(f, "ramka iiD %d\n", ramka.iID);

				/*if (moj_pojazd->doZmiany) {
					fprintf(f, "Do zmiany %d\n", moj_pojazd->doZmiany->iID);
				}*/

				if (moj_pojazd->czy_gotowy_na_zmiane && moj_pojazd->iID == ramka.doZmiany && moj_pojazd->doZmiany == ramka.iID) {

					fprintf(f, "Zamiana true %d\n", moj_pojazd->iID);


					//wysy³anie ramki o zamianie do pierwszego gracza
					Ramka ramka2;
					ramka2.typ = ZAMIANA;
					ramka2.doZmiany = moj_pojazd->doZmiany;
					ramka2.iID = moj_pojazd->iID;
					multi_send->send((char*)&ramka2, sizeof(Ramka));
					//zamiana u mnie
					ObiektRuchomy* ob = moj_pojazd;
					moj_pojazd = obiekty_ruchome[ramka.iID];
				    obiekty_ruchome[ramka.doZmiany] = ob;
					moj_pojazd->czy_gotowy_na_zmiane = 0;
...
void Cykl_WS()
{
	...
	{                                   // nale¿y na nowo obliczyæ œredni czas cyklu fDt
		...
		sprintf(text, "WZR-lab zima 2021/22 temat 1, wersja c (%0.0f fps  %0.2fms)  ", fFps, 1000.0 / fFps);
		SetWindowText(okno, text); // wyœwietlenie aktualnej iloœci klatek/s w pasku okna			
	}
	if (moj_pojazd->czy_gotowy_na_zmiane) {                     //je¿eli jestem gotowy na zmianê wysy³am specjaln¹ ramkê aby poinformowaæ o tym u¿ytkowników
		Ramka ramka;
		ramka.typ = ZAMIANA;
		ramka.doZmiany = moj_pojazd->doZmiany;
	...
	int       nCmdShow)
{
	...
	okno = CreateWindow(nazwa_klasy, "WZR-lab zima 2021/22 temat 1, wersja c", WS_OVERLAPPEDWINDOW | WS_VISIBLE | WS_CLIPCHILDREN | WS_CLIPSIBLINGS,
	...
	case WM_CHAR:
	{
		if (wParam >= '0' && wParam <= '9') {
			...
			for (map<int, ObiektRuchomy*>::iterator it = obiekty_ruchome.begin(); it != obiekty_ruchome.end(); it++)
			{
				...
				if (it->second->iID != moj_pojazd->iID && '0' + it->second->iID % 10 == wParam) {
					moj_pojazd->doZmiany = it->second->iID;    //pojazd, z którym chcemy siê zamieniæ
					moj_pojazd->czy_gotowy_na_zmiane = 1;  // oznaczenie, ¿e jesteœmy gotowi na zmianê
